...
function createHomepageGoogleMap(_latitude,_longitude,json){
    $.get("views/assets/external/_infobox.js", function() {
        gMap();
    });
    ...
    function gMap(){
        ...
        for (var i = 0; i < json.length; i++) {
          ...
          // alert(json[i].geoCordinates.latitude);
          ...
          /*  if( json[i].color ) var color = json[i].color;
            ...
            */
            ...
            var markerContent = document.createElement('DIV');
                ...
                markerContent.innerHTML =
                    '<div class="map-marker ' + json[i].color + '">' +
                        '<div class="icon">' +
                        '<img src="views/assets/icons/store/apparel/clothers_male.png">' +
                        '</div>' +
                    '</div>';
            ...
            var marker = new RichMarker({
                position: new google.maps.LatLng(json[i].geoCordinates.latitude, json[i].geoCordinates.longitude ),
                map: map,
                draggable: false,
            ...
            var infoboxOptions = {
                ...
                closeBoxURL: "views/assets/img/close.png",
        ...
        var clusterStyles = [
            {
                url: 'views/assets/img/cluster.png',
                height: 34,
                width: 34
            }
        ];
        ...
        google.maps.event.addListener(map, 'idle', function() {
            ...
            $.each(json, function(a) {
                if( map.getBounds().contains( new google.maps.LatLng( json[a].geoCordinates.latitude, json[a].geoCordinates.longitude ) ) ) {
                    var category = json[a].type;
...
else icon = '<img src="views/assets/icons/store/apparel/clothers_male.png">';
*/
            ...
            */
            ...
            var marker = L.marker(new L.LatLng( json[i].geoCordinates.latitude, json[i].geoCordinates.longitude ), {
                title: title,
                icon: _icon
            });
...
function itemDetailMap(json){
    var mapCenter = new google.maps.LatLng(json.geoCordinates.latitude,json.geoCordinates.longitude);
    var mapOptions = {
        zoom: 14,
        center: mapCenter,
  ...
  /* if( json.type_icon ) var icon = '<img src="' + json.type_icon +  '">';
    ...
    */
    ...
    var marker = new RichMarker({
        position: new google.maps.LatLng( json.geoCordinates.latitude, json.geoCordinates.longitude ),
        map: map,
        draggable: false,
        content: markerContent,
        flat: true
...
function pushItemsToArray(json, a, category, visibleItemsArray){
    ...
    visibleItemsArray.push(
        '<li>' +
            '<div class="item" id="' + json[a]._id + '">' +
                '<a href="#/listings/'+json[a]._id+'" class="image">' +
                    '<div class="inner">' +
                ...
                '</a>' +
                '<div class="wrapper">' +
                    '<a href="#/listings/'+json[a]._id+'" class="modal-exit"><h3>' + json[a].name + '</h3></a>' +
                    '<figure>' + json[a].contact.address + '<br/>' + json[a].contact.country + '</figure>' +
                   // drawPrice(json.data[a].price) +
                    '<div class="info">' +
                        '<div class="type">' +
                            '<i><img src="views/assets/icons/store/apparel/clothers_male.png" alt=""></i>' +
...
function centerMapToMarker(){
    $.each(json, function(a) {
        if( json[a]._id == id ) {
            var _latitude = json[a].geoCordinates.latitude;
            var _longitude =json[a].geoCordinates.longitude;
            var mapCenter = new google.maps.LatLng(_latitude,_longitude);
...
function multiChoice(sameLatitude, sameLongitude, json) {
    //if (clickedCluster.getMarkers().length > 1){
		...
		//alert('test');
		//console.log(json);
		//alert(sameLatitude+" "+sameLongitude);
		
        $.each(json, function(a) {
			//alert(json[a].geoCordinates.latitude+" "+json[a].geoCordinates.longitude);
			
			//var precision =  Math.pow(10, 6);
			//var new_Latitude = double((int)(precision * json[a].geoCordinates.latitude))/precision;
			latitude=json[a].geoCordinates.latitude;
			latitude = latitude.substring(0,latitude.indexOf(".")+6);
			longitude=json[a].geoCordinates.longitude;
			longitude = longitude.substring(0,longitude.indexOf(".")+6);
			
			sameLatitude = sameLatitude.substring(0,sameLatitude.indexOf(".")+6);
			sameLongitude = sameLongitude.substring(0,sameLongitude.indexOf(".")+6);
			
			//alert(latitude+longitude);
			
            if(latitude == sameLatitude && longitude == sameLongitude) {
              // alert('hi');
			   pushItemsToArray(json, a, json[a].type, multipleItems);
            }
        });
		
		//document.write(multipleItems);
        $('body').append('<div class="modal-window multichoice fade_in"></div>');
        $('.modal-window').load( 'views/assets/external/_modal-multichoice.html', function() {
            $('.modal-window .modal-wrapper .items').html( multipleItems );
            rating('.modal-window');
        });
        $('.modal-window .modal-background, .modal-close,.modal-exit').live('click',  function(e){
...
function redrawMap(mapProvider, map){
    $('.map .toggle-navigation').click(function() {
		...
		if($( ".map-canvas" ).hasClass( "results-collapsed" ))
		{
			...
			$( ".map-canvas" ).removeClass( "results-collapsed" );
			$( ".map-canvas" ).addClass( "list-width-30" );
			//alert('test1');
			
		}
		
		if($( ".map-canvas" ).hasClass( "list-width-30" ))
		{
			$( ".map-canvas" ).removeClass( "list-width-30" );
			$( ".map-canvas" ).addClass( "results-collapsed" );
			//alert('test2');
