...
// Set width for inputs in horizontal search bar -----------------------------------------------------------------------
    ...
    $( "#redefine-search-form" ).load( "views/assets/external/_search-bar.html", function() {
        setInputsWidth();
        //autoComplete();
    });
...
function autoComplete(){
    if( !$("script[src='views/assets/js/leaflet.js']").length ){
...
function drawOwlCarousel(_rtl){
    $.getScript( "views/assets/js/owl.carousel.min.js", function( data, textStatus, jqxhr ) {
...
function drawItemSpecific(category, json, a){
    ...
    if( category ){
        if( category == 'real_estate' ){
            if( json.data[a].item_specific ){
                if( json.data[a].item_specific.bedrooms ){
                    itemSpecific += '<span title="Bedrooms"><img src="views/assets/img/bedrooms.png">' + json.data[a].item_specific.bedrooms + '</span>';
                }
                if( json.data[a].item_specific.bathrooms ){
                    itemSpecific += '<span title="Bathrooms"><img src="views/assets/img/bathrooms.png">' + json.data[a].item_specific.bathrooms + '</span>';
                }
                if( json.data[a].item_specific.area ){
                    itemSpecific += '<span title="Area"><img src="views/assets/img/area.png">' + json.data[a].item_specific.area + '<sup>2</sup></span>';
                }
                if( json.data[a].item_specific.garages ){
                    itemSpecific += '<span title="Garages"><img src="views/assets/img/garages.png">' + json.data[a].item_specific.garages + '</span>';
                }
                return itemSpecific;
            }
        }
	 ...
	 //return '';
	 itemSpecific += '<img src="views/assets/img/items/1.jpg" alt="" class="img-responsive">';
    return itemSpecific ;
...
function quickView(id){
    $.ajax({
        type: 'POST',
        url: 'views/assets/external/_modal.html',
        data: id,
